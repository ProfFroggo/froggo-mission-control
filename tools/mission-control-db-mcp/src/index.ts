...
/** Awaitable API call — supports GET, POST, PATCH. Returns parsed JSON. */
function apiCall(method: string, urlPath: string, body?: object): Promise<any> {
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description: 'Create a new phase in a project plan. Use for GSD-style structured planning — break large work into ordered phases with clear goals.',
      inputSchema: {
        type: 'object',
        properties: {
          projectId: { type: 'string', description: 'Project ID' },
          title: { type: 'string', description: 'Phase title (e.g., "Phase 1: Research & Discovery")' },
          description: { type: 'string', description: 'Phase goal and key deliverables' },
          assignedTo: { type: 'string', description: 'Agent ID to assign this phase to' },
        },
    ...
    },
    {
      ...
      description: 'Update a project phase status or details. Mark phases as in-progress when starting, complete when done.',
      inputSchema: {
        type: 'object',
        properties: {
          phaseId: { type: 'string', description: 'Phase ID to update' },
          projectId: { type: 'string', description: 'Project ID (for API routing)' },
          status: { type: 'string', enum: ['planned', 'in-progress', 'complete'], description: 'Phase status' },
          title: { type: 'string', description: 'Updated title' },
          description: { type: 'string', description: 'Updated description' },
          assignedTo: { type: 'string', description: 'Reassign to different agent' },
        },
  ...
  try {
    switch (name) {
      ...
      case 'project_phase_list': {
        ...
        const res = await apiCall('GET', `/api/projects/${encodeURIComponent(projectId)}/phases`);
      ...
      case 'project_phase_create': {
        ...
        for (const key of ['title', 'description', 'assignedTo']) {
          if (args?.[key]) body[key] = args[key];
        }
        const res = await apiCall('POST', `/api/projects/${encodeURIComponent(projectId)}/phases`, body);
        return { content: [{ type: 'text', text: JSON.stringify(res) }] };
      }
      ...
      case 'project_phase_update': {
        ...
        if (!projectId || !phaseId) return { content: [{ type: 'text', text: JSON.stringify({ error: 'projectId and phaseId are required' }) }], isError: true };
        const body: Record<string, unknown> = {};
        for (const key of ['title', 'description', 'status', 'assignedTo']) {
          if (args?.[key]) body[key] = args[key];
        }
        const res = await apiCall('PATCH', `/api/projects/${encodeURIComponent(projectId)}/phases?phaseId=${encodeURIComponent(phaseId)}`, body);
        return { content: [{ type: 'text', text: JSON.stringify(res) }] };
      }
