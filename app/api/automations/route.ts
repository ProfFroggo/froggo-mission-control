...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    db.prepare(`
      INSERT INTO automations (id, name, description, trigger_type, trigger_config, steps, status, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      id,
      name,
...
export async function PATCH(request: NextRequest) {
  ...
  try {
    ...
    const allowed = ['name', 'description', 'trigger_type', 'trigger_config', 'steps', 'status', 'next_run', 'last_run'];
