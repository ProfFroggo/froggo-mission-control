...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const links = db.prepare('SELECT * FROM knowledge_base_links WHERE knowledgeId = ? ORDER BY createdAt ASC').all(params.id);
  const assets = db.prepare('SELECT * FROM knowledge_base_assets WHERE knowledgeId = ? ORDER BY createdAt ASC').all(params.id);
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (body.links !== undefined) {
    db.prepare('DELETE FROM knowledge_base_links WHERE knowledgeId = ?').run(params.id);
    for (const link of body.links as Array<{ url?: string; title?: string; description?: string }>) {
      ...
      db.prepare('INSERT INTO knowledge_base_links (id, knowledgeId, title, url, description, createdAt) VALUES (?, ?, ?, ?, ?, ?)').run(
        lid, params.id, link.title || link.url, link.url, link.description || '', Date.now()
      );
    }
  }
