# Requirements Archive: v3.0 App-Wide Polish & X/Twitter Overhaul
...
- [x] **UI-01**: All text inputs use dark background + light text in dark mode (no white-on-white)
- [x] **UI-02**: Agents page more-dropdown button border uses theme color (not white)
- [x] **UI-03**: Agents page divider lines above/below dropdown buttons use theme color (not white)
- [x] **UI-04**: User message chat bubbles use 50% opacity green background (not full bright green)
- [x] **UI-05**: All chat dialogues across every page use the same component and styling (Chat page style as reference)
- [x] **UI-06**: Chat input bar bottom alignment consistent across all pages

### X/Twitter — Identity & Dark Mode

- [x] **XTW-01**: Twitter bird icon replaced with X logo everywhere on X/Twitter page
- [x] **XTW-02**: All inputs, dropdowns, cards, and backgrounds on X/Twitter page are dark-mode styled
- [x] **XTW-03**: Page label uses "X / Twitter" not "Twitter"

### X/Twitter — Tab Structure

- [x] **XTW-04**: Tab nav order: Content Plan → Drafts → Calendar → Mentions → Reply Guy → Content Mix Tracker → Automations → Analytics
- [x] **XTW-05**: Approval queue side panel removed from Calendar tab
- [x] **XTW-06**: Approval queue side panel removed from Mentions tab
- [x] **XTW-07**: Approval queue side panel removed from Reply Guy tab
- [x] **XTW-08**: Approval queue side panel removed from Automations tab
...
- [x] **XTW-25**: Automations has chat interface with agent (existing, keep)
- [x] **XTW-26**: Automation builder UI rebuilt (visual rule builder)
- [x] **XTW-27**: No approval panel on automations tab

### X/Twitter — Analytics

- [x] **XTW-28**: Analytics tab exists at end of nav
- [x] **XTW-29**: Analytics shows total breakdown: posts, engagement, reach, top content
- [x] **XTW-30**: Analytics includes competitor insights section
- [x] **XTW-31**: Daily insights report downloadable as text file

### Writing Module — Pane Layout

- [x] **WRT-01**: Left double-bar (pane 1 + pane 2 handles) visible and functional on first load
- [x] **WRT-02**: Pane 1 (chapters list) has usable min-width (≥180px) and renders content on first open
- [x] **WRT-03**: Pane 2 (AI chat) has usable min-width (≥280px) and renders content on first open
- [x] **WRT-04**: Panel width adjustments via drag handle work across the full usable range
...
**Shipped:** 50 of 50 v3.0 requirements
**Adjusted:** None — all requirements shipped as defined
**Dropped:** None

---
*Archived: 2026-02-18 as part of v3.0 milestone completion*
