...
async function cmdSetup(force = false) {
  ...
  // Resolve CLAUDE_SCRIPT — the actual JS entry point used when spawning via `node`
  let claudeScript = existingEnv.CLAUDE_SCRIPT || '';
  if (!claudeScript || !existsSync(claudeScript)) {
    try {
      claudeScript = require.resolve('@anthropic-ai/claude-code');
    } catch {
      // Fallback: cli.js lives next to the claude binary
      const claudeDir = path.dirname(require('fs').realpathSync(resolvedClaude));
      const candidate = path.join(claudeDir, 'cli.js');
      claudeScript = existsSync(candidate) ? candidate : '';
    }
  }
  ...
  const envContent = [
    ...
    '',
    '# Internal security token (auto-generated, unique per install — never commit)',
  ...
  const envVars = {
    ...
    LOG_DIR: MC_LOGS,
    GEMINI_API_KEY: geminiKey,
    // NOTE: ANTHROPIC_API_KEY intentionally omitted — Claude Code CLI uses its own
    // stored credentials (keychain/config). Setting it here overrides that lookup
    // and causes apiKeySource:"none", breaking all agent chat.
...
async function cmdUpdate() {
  info('Updating froggo-mission-control to latest...');
  // Use `npm install -g` not `npm update -g` — install always pulls latest from registry
  const r = spawnSync('npm', ['install', '-g', 'froggo-mission-control@latest'], { stdio: 'inherit' });
  if (r.status !== 0) fail('Update failed');
  // Rebuild with new files
  await cmdBuild();
  await cmdRestart();
  success('Updated to latest and restarted');
}
