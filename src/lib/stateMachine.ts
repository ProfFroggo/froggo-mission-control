...
/**
 * Task Pipeline State Machine — Next.js / Vitest edition
 *
 * This is the canonical pure-logic mirror of
 * tools/mission-control-db-mcp/src/stateMachine.ts.
 *
 * Both files contain identical business logic. Keep them in sync:
 * the MCP version is compiled to CommonJS for the MCP server process,
 * while this version is used by the Next.js app and the Vitest test suite.
 *
 * Design principles:
