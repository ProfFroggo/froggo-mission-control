  ...
  return (
    <div className="space-y-1">
      <div className="flex items-center justify-between text-xs">
        ...
        <div className="flex items-center gap-2 text-mission-control-text-dim shrink-0">
          <span>{tokLabel}</span>
...
export default function TokenUsageWidget() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/token-usage?days=${period}`);
      if (res.ok) {
  ...
  return (
    <div className="h-full flex flex-col gap-5 overflow-y-auto">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex bg-mission-control-border rounded-lg p-0.5">
          {(['7', '30'] as const).map(p => (
            <button
              key={p}
              type="button"
      ...
      {/* Budget alert banner (only on monthly view) */}
      {period === '30' && <BudgetBanner totalCost={data.totalCost} budgetUsd={budgetUsd} />}
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-mission-control-surface border border-mission-control-border rounded-xl p-4">
          ...
          <div className="text-xs text-mission-control-text-dim mt-0.5">last {period} days</div>
        </div>
        ...
        <div className="bg-mission-control-surface border border-mission-control-border rounded-xl p-4">
          ...
          <div className="text-2xl font-bold text-success">${data.totalCost.toFixed(4)}</div>
      ...
      {top3.length === 0 && data.byDay.length === 0 && (
        <div className="flex-1 flex items-center justify-center text-sm text-mission-control-text-dim">
          No token usage recorded in the last {period} days
        </div>
      )}
