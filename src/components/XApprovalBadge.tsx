...
function getTypeInfo(type: string): { label: string; icon: React.ReactNode; color: string } {
  switch (type) {
    case 'tweet':
      return { label: 'Tweet', icon: <Send size={12} />, color: 'text-[var(--color-info)]' };
    case 'x-reply':
      return { label: 'Reply', icon: <MessageSquare size={12} />, color: 'text-[var(--color-success)]' };
    case 'research':
      return { label: 'Research', icon: <Zap size={12} />, color: 'text-[var(--color-warning)]' };
    case 'plan':
      return { label: 'Plan', icon: <FileText size={12} />, color: 'text-[var(--color-review)]' };
    case 'draft':
      return { label: 'Draft', icon: <FileText size={12} />, color: 'text-[var(--color-info)]' };
    default:
    ...
    <div className="relative" ref={dropdownRef}>
      ...
      <button
        ...
        className={`relative flex items-center gap-1.5 px-2.5 py-1.5 text-sm rounded-lg border transition-colors ${
          open
            ? 'bg-[var(--color-warning)]/10 border-[var(--color-warning)]/30 text-[var(--color-warning)] font-medium'
            : count > 0
            ? 'bg-[var(--color-warning)]/10 border-[var(--color-warning)]/20 text-[var(--color-warning)] hover:border-[var(--color-warning)]/40'
      ...
      >
        <ShieldAlert size={15} />
        {count > 0 && (
          <span className="flex items-center justify-center min-w-[18px] h-[18px] px-1 text-xs font-bold bg-[var(--color-warning)]/15 text-[var(--color-warning)] rounded-full tabular-nums">
            {count}
          </span>
        )}
      </button>
                          ...
                          <div className="mb-2">
                            ...
                            <p className="text-xs text-mission-control-text line-clamp-2">
                              {preview.isAutoGenerated && (
                                <span className="text-xs font-bold px-1 py-0.5 rounded bg-[var(--color-info)]/20 text-[var(--color-info)] mr-1">AI</span>
                              )}
                              {preview.main}
                            </p>
                          </div>
