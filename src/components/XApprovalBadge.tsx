...
function getPreview(item: PendingApproval): { main: string; context?: string; isAutoGenerated?: boolean } {
  ...
  if (item.type === 'x-reply' && item.metadata?.auto_generated) {
    return {
      main: item.payload?.replyText || 'Suggested reply',
  ...
  if (item.payload?.replyText) return { main: item.payload.replyText };
  if (item.metadata?.title) return { main: item.metadata.title };
