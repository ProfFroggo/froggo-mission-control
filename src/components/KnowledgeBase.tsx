...
export default function KnowledgeBase() {
  ...
  const [customCategories, setCustomCategories] = useState<string[]>(() => {
    try { return JSON.parse(localStorage.getItem('kb-custom-categories') || '[]'); } catch { return []; }
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Don't filter by category server-side — we need all articles for accurate counts
  ...
  // Client-side filtered articles for display
  ...
  const allCategories = useMemo<string[]>(() => {
    ...
    return [...new Set([...DEFAULT_CATEGORIES, ...customCategories, ...fromArticles])].sort();
  }, [articles, customCategories]);

  const handleAddCategory = useCallback((cat: string) => {
    if (!cat || DEFAULT_CATEGORIES.includes(cat) || customCategories.includes(cat)) return;
    const updated = [...customCategories, cat];
    setCustomCategories(updated);
    localStorage.setItem('kb-custom-categories', JSON.stringify(updated));
  }, [customCategories]);
  ...
  // ── Filtered articles (category + starred filter + search highlight) ──────
  ...
  const save = async () => {
    ...
    // If category changed, switch view to the new category so the article is visible
    if (editing.category && editing.category !== category) {
  ...
  return (
    ...
    >
      {/* Drop overlay */}
      {isDragOver && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-mission-control-bg/80 pointer-events-none">
          <div className="text-center">
            <Upload size={48} className="mx-auto mb-3 text-mission-control-accent" />
            <p className="text-lg font-semibold text-mission-control-accent">Drop file to add to Knowledge Base</p>
            <p className="text-sm text-mission-control-text-dim mt-1">Gemini will analyze, categorize, and tag it automatically</p>
          </div>
        </div>
      )}
          ...
          <div className="flex items-start gap-2">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap gap-1 mt-1">
                {ingestResult.tags.slice(0, 5).map(t => (
                  <span key={t} className="text-xs px-1.5 py-0.5 bg-mission-control-border/50 rounded text-mission-control-text-dim">{t}</span>
                ))}
              </div>
            </div>
...
function CategorySidebar({ categories, counts, selected, total, onSelect, onAddCategory }: CategorySidebarProps) {
          ...
          <input
            ...
            onKeyDown={e => {
              if (e.key === 'Enter' && newCat.trim()) {
                const cat = newCat.trim().toLowerCase().replace(/\s+/g, '-');
                onAddCategory?.(cat);
                onSelect(cat);
                setNewCat('');
                setAdding(false);
              }
              if (e.key === 'Escape') { setNewCat(''); setAdding(false); }
            }}
