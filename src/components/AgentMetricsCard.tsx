...
const ProgressBar = ({ value, max, color }: { value: number; max: number; color: string }) => {
  ...
  return (
    <div className="w-full bg-mission-control-bg rounded-full h-1.5 overflow-hidden mt-1">
      <div className={`h-full ${color} transition-colors duration-500`} style={{ width: `${pct}%` }} />
    </div>
  );
};
...
const Stat = ({ icon: Icon, label, value, color = 'text-mission-control-text', sub }: {
  ...
  <Box className="bg-mission-control-bg rounded-lg p-3">
    ...
    <div className={`text-2xl font-bold tabular-nums font-mono ${color}`}>{value}</div>
...
function OrchestratorMetrics({ m, compact }: { m: Record<string, number>; compact: boolean }) {
  if (compact) {
    return (
      <Flex align="center" gap="3" className="text-sm overflow-hidden">
        <Flex align="center" gap="1" className="text-mission-control-text-dim no-shrink">
          <Users size={13} />
  ...
  return (
    <div className="space-y-3">
      <Flex align="center" justify="between">
        <h4 className="text-[10px] font-bold text-mission-control-text-dim uppercase tracking-wider">Orchestration Metrics</h4>
...
function QCMetrics({ m, compact }: { m: Record<string, number>; compact: boolean }) {
  ...
  if (compact) {
    return (
      <Flex align="center" gap="3" className="text-sm overflow-hidden">
        <Flex align="center" gap="1" px="2" py="1" className={`rounded-lg border no-shrink ${ratingBg}`}>
          <Shield size={13} />
  ...
  return (
    <div className="space-y-3">
      <Flex align="center" justify="between">
        <h4 className="text-[10px] font-bold text-mission-control-text-dim uppercase tracking-wider">Quality Control</h4>
...
export default function AgentMetricsCard({ agentId, agentName: _agentName, metrics, compact = false }: AgentMetricsCardProps) {
  ...
  if (compact) {
    return (
      <Flex align="center" gap="3" className="text-sm overflow-hidden">
        <Flex align="center" gap="1" px="2" py="1" className={`rounded-lg border no-shrink no-wrap ${rating.color}`}>
          <Target size={14} className="no-shrink" />
          <span className="font-semibold no-shrink">{m.completionRate.toFixed(1)}%</span>
        </Flex>
        <Flex align="center" gap="1" className="text-mission-control-text-dim no-shrink no-wrap">
  ...
  return (
    <div className="space-y-4 overflow-hidden">
      <Flex align="center" justify="between" gap="2" className="min-w-0">
        <h4 className="text-[10px] font-bold text-mission-control-text-dim uppercase tracking-wider flex-shrink">
          Performance Metrics
        </h4>
