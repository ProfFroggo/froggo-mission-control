...
export default function LibraryFilesTab({ initialPath }: LibraryFilesTabProps = {}) {
  ...
  const folderFiltered = applyFolderFilter(categoryFiltered);
  const filteredFiles = searchMode === 'filter'
    ? folderFiltered.filter(f =>
        f.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
        f.tags?.some(t => t.toLowerCase().includes(searchQuery.toLowerCase()))
      )
    : folderFiltered; // In ask mode, show all but agent filters via response
