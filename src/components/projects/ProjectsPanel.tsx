...
function StatsStrip({ projects }: StatsStripProps) {
  ...
  const chips = [
    { label: 'Total', value: total, icon: FolderKanban, color: 'text-mission-control-text-primary' },
...
function ProjectCard({ project, onClick, onArchive, onRestore }: ProjectCardProps) {
  ...
  return (
    <div className="group relative w-full bg-mission-control-surface border border-mission-control-border rounded-xl hover:border-mission-control-accent/50 transition-all duration-200">
      ...
      <button
        onClick={onClick}
        className="w-full text-left p-5 focus:outline-none focus:ring-2 focus:ring-mission-control-accent/30 rounded-xl"
      >
        {/* Header */}
...
export default function ProjectsPanel() {
  ...
  return (
    <div className="flex flex-col h-full bg-mission-control-bg0">
      ...
      {/* Stats strip */}
      {!loading && !error && nonArchivedProjects.length > 0 && (
        <StatsStrip projects={nonArchivedProjects} />
      )}
