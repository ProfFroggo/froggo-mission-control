...
CLAUDE_SCRIPT="$(node -e "require.resolve('@anthropic-ai/claude-code')" 2>/dev/null || true)"
if [ -z "${CLAUDE_SCRIPT}" ]; then
  # Fallback: find cli.js relative to the binary
  CLAUDE_SCRIPT="$(dirname "$(realpath "${CLAUDE_BIN}")")/cli.js"
fi
success "Claude Code CLI: $CLAUDE_BIN"
success "Claude Code script: $CLAUDE_SCRIPT"
...
# Generate a unique internal API token for this installation (never checked in)
INTERNAL_API_TOKEN="$(openssl rand -hex 32 2>/dev/null || node -e "require('crypto').randomBytes(32).toString('hex').split('').forEach(c=>process.stdout.write(c))")"
...
if [ "${OS}" = "Darwin" ]; then
  ...
  # Resolve absolute node binary path — avoids nvm/fnm shims that don't work in LaunchAgent
  NODE_BIN="$(realpath "$(which node 2>/dev/null)" 2>/dev/null || echo /opt/homebrew/bin/node)"
  if [ ! -x "${NODE_BIN}" ]; then
    NODE_BIN="$(find /opt/homebrew/bin /usr/local/bin -name node -type f 2>/dev/null | head -1 || echo /opt/homebrew/bin/node)"
  fi
...
<dict>
  ...
  <dict>
    ...
    <key>CLAUDE_SCRIPT</key>
    <string>${CLAUDE_SCRIPT}</string>
...
EOF
  ...
  NODE_BIN="$(realpath "$(which node)" 2>/dev/null || which node)"
