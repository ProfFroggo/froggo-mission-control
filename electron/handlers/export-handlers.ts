...
export function registerExportHandlers(): void {
  registerHandler('export:tasks', async (_event, options: { format: 'json' | 'csv'; filters?: Record<string, unknown> }) => {
    try { safeLog.log('[Export] Exporting tasks with format:', options.format); return { success: true, filepath: await exportTasks(options) }; }
    catch (error: unknown) { safeLog.error('[Export] Task export error:', error); return { success: false, error: error.message }; }
  });

  registerHandler('export:agentLogs', async (_event, options: { format: 'json' | 'csv'; filters?: Record<string, unknown> }) => {
    try { safeLog.log('[Export] Exporting agent logs'); return { success: true, filepath: await exportAgentLogs(options) }; }
    catch (error: unknown) { safeLog.error('[Export] Agent logs export error:', error); return { success: false, error: error.message }; }
  });

  registerHandler('export:chatHistory', async (_event, options: { format: 'json' | 'csv'; filters?: Record<string, unknown> }) => {
    try { safeLog.log('[Export] Exporting chat history'); return { success: true, filepath: await exportChatHistory(options) }; }
    catch (error: unknown) { safeLog.error('[Export] Chat history export error:', error); return { success: false, error: error.message }; }
  });
  ...
  registerHandler('backup:create', async (_event, options?: { includeAttachments?: boolean }) => {
    ...
    catch (error: unknown) { safeLog.error('[Backup] Create backup error:', error); return { success: false, error: error.message }; }
  });
  ...
  registerHandler('backup:restore', async (_event, backupPath: string) => {
    ...
    catch (error: unknown) { safeLog.error('[Backup] Restore error:', error); return { success: false, error: error.message }; }
  });

  registerHandler('backup:list', async () => {
    try { return { success: true, backups: await listBackups() }; }
    catch (error: unknown) { safeLog.error('[Backup] List backups error:', error); return { success: false, backups: [], error: error.message }; }
  });
  ...
  registerHandler('backup:cleanup', async (_event, keepCount: number) => {
    ...
    catch (error: unknown) { safeLog.error('[Backup] Cleanup error:', error); return { success: false, error: error.message }; }
  });
  ...
  registerHandler('import:tasks', async (_event, filepath: string) => {
    ...
    catch (error: unknown) { safeLog.error('[Import] Import error:', error); return { success: false, error: error.message }; }
  });

  registerHandler('exportBackup:stats', async () => {
    try { return { success: true, stats: await getStats() }; }
    catch (error: unknown) { safeLog.error('[ExportBackup] Stats error:', error); return { success: false, stats: null, error: error.message }; }
  });
  ...
  registerHandler('hrReports:list', async () => {
    ...
    } catch (error: unknown) { safeLog.error('[HRReports] List error:', error); return { success: false, reports: [], error: error.message }; }
  });
  ...
  registerHandler('hrReports:read', async (_event, filename: string) => {
    ...
    } catch (error: unknown) { safeLog.error('[HRReports] Read error:', error); return { success: false, error: error.message }; }
  });
}
