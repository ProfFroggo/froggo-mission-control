...
export function registerFinanceAgentHandlers(): void {
  registerHandler('financeAgent:sendMessage', async (_event, message: string, context?: any) => {
    try {
    ...
    } catch (error: any) { safeLog.error('[FinanceAgent] Send message error:', error.message); return { success: false, error: error.message }; }
  });
  ...
  registerHandler('financeAgent:getChatHistory', async () => {
    ...
    catch (error: any) { safeLog.error('[FinanceAgent] Get chat history error:', error.message); return { success: false, messages: [], error: error.message }; }
  });
  ...
  registerHandler('financeAgent:clearHistory', async () => {
    ...
    catch (error: any) { safeLog.error('[FinanceAgent] Clear history error:', error.message); return { success: false, error: error.message }; }
  });
  ...
  registerHandler('financeAgent:triggerAnalysis', async (_event, analysisType?: 'csv_upload' | 'manual') => {
    ...
    } catch (error: any) { safeLog.error('[FinanceAgent] Trigger analysis error:', error.message); return { success: false, error: error.message }; }
  });
  ...
  registerHandler('financeAgent:getStatus', async () => {
    ...
    catch (error: any) { safeLog.error('[FinanceAgent] Get status error:', error.message); return { success: false, error: error.message }; }
  });

  registerHandler('dashboardAgents:status', async () => {
    try { return { success: true, agents: getDashboardAgentsStatus() }; }
    catch (error: any) { safeLog.error('[DashboardAgents] Status error:', error); return { success: false, agents: [], error: error.message }; }
  });
}
